...
const IN_PROGRESS_STATUSES = ['pending', 'preprocessing', 'generating_image', 'generating_video']
...
function groupByBatch(generations: Generation[]): BatchInfo[] {
  ...
  // Singles that are in-progress also count
  groups.push(...singles.filter(s =>
    IN_PROGRESS_STATUSES.includes(s.generations[0].status) ||
    s.generations[0].status === 'failed'
  ))
...
function BatchProgressBar({ batch }: { batch: BatchInfo; onRetry: (id: number) => void }) {
...
export function BatchProgress({ projectId, refreshTrigger }: BatchProgressProps) {
  ...
  const loadGenerations = useCallback(async () => {
    try {
      // Load recent generations (enough to cover active batches)
